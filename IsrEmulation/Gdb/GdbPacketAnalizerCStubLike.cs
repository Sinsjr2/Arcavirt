...
public class GdbPacketAnalizerCStubLike {
    ...
    // static async ValueTask<char> ReadChar(TextReader rx, CancellationToken token) {
    //     var pool = ArrayPool<char>.Shared;
    //     char[]? buf = null;
    //     try {
    //         buf = pool.Rent(1);
    //         if (await rx.ReadBlockAsync(buf.AsMemory(0, 1), token) == 0) {
    //             throw new OperationCanceledException("reached to end");
    //         }
    //         Console.Write(buf[0]);
    //         return buf[0];
    //     } finally {
    ...
    // static async ValueTask WriteChar(TextWriter tx, char value, CancellationToken token) {
    //     var pool = ArrayPool<char>.Shared;
    //     char[]? buf = null;
    //     try {
    //         buf = pool.Rent(1);
    //         buf[0] = value;
    //         await tx.WriteAsync(buf.AsMemory(0, 1), token);
    //         await tx.FlushAsync(token);
    //     } finally {
    //         if (buf != null) {
    //             pool.Return(buf);
    //         }
    ...
    // static async ValueTask PutPacket(TextWriter tx, TextReader rx, string cmd, CancellationToken token) {
    //     var sb = new StringBuilder();
    //     GDBUtils.GDBMessage(sb, cmd);
    //     while (true) {
    //         await tx.WriteAsync(sb, token);
    //         await tx.FlushAsync(token);
    //         if (await ReadChar(rx, token) == '+') {
    //             break;
    //         }
    //     }
    // }
    ...
    // /// <summary>
    // /// result 解析したコマンドを詰め込みます。
    // /// 初めにresult に値が入っていてもクリアします。
    // /// </summary>
    // static async ValueTask GetPacket(TextWriter tx, TextReader rx, StringBuilder result, int bufferSize, CancellationToken token) {
    //     var pool = ArrayPool<char>.Shared;
    //     char[]? checkSumBuf = null;
    //     try {
    //         checkSumBuf = pool.Rent(2);

    //         while (true) {
    //             result.Clear();
    //             var firstMessage = await ReadChar(rx, token);
    //             if (firstMessage == GDBMessage.Break) {
    ...
    //                 // パケット終端文字を発見
    //                 // チェックサムと一致するかを確認する
    //                 checkSumBuf[0] = await ReadChar(rx, token);
    //                 checkSumBuf[1] = await ReadChar(rx, token);
    //                 var checkSumA = GDBUtils.GDBChecksum(result.GetChunks());
    //                 if (!GDBUtils.TryDecodeHexU8(checkSumBuf.AsSpan(0, 2), out var checkSumB, out _) ||
    //                     checkSumA != checkSumB) {
    //                     // チェックサム不一致再送要求
    //                     Console.WriteLine();
    //                     Console.WriteLine($"check sum invalid  msg: {result}");
    //                     Console.WriteLine($"check sum a: {checkSumA}, check sum b: {checkSumB}");
    //                     await WriteChar(tx, '-', token);
    //                     continue;
    //                 }
    //                 // Note: シーケンス番号非対応
    //                 await WriteChar(tx, '+', token);
