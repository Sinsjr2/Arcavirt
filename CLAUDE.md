...
## ブランチ命名
- `doc/branch-guidelines.md` に従う。

## .beadsの同時利用について
- `.beads` はembeddedモードのDolt(サーバーなし)で動作しており、複数プロセスからの
  同時書き込みを調停する仕組みがない。
- ホストと`docker compose`の`dev`コンテナ(claude code常駐用)は同じ`.beads`
  ディレクトリを共有しているため、どちらか一方でのみ`bd`を使う運用とする
  (同時に使わない)。
