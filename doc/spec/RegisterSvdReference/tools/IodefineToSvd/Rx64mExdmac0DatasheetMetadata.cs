...
/// チャネル0のレジスタ群(st_exdmac0、インスタンスEXDMAC0)のレジスタごとの
/// リセット値・アクセス権限。EDMSAR・EDMDARはvoid*ポインタ型メンバー(32ビットの
/// アドレス設定レジスタ)で、本文にビット単位のBit/Description表があり全ビットR/Wと
/// 明記されているため、通常のレジスタと同様にread-writeとした。EDMOFRは
/// EXDMAC0にのみ存在するレジスタ(19.2.9節、EDMAMD本文にも「Offset addition can be
/// specified only for EXDMAC0」と明記)。
///
/// EDMSTS(19.2.12節)はESIF・DTIFビットがR/W(0書き込みでクリア)、ACT・予約ビットが
/// Rのみのため、書込み可能なビットが存在するread-writeとした(DMACのDMSTSと同様の
/// 構成)。
/// </summary>
public static class Rx64mExdmac0DatasheetMetadata {
    public static IReadOnlyDictionary<string, RegisterDatasheetMetadata> Registers => new Dictionary<string, RegisterDatasheetMetadata> {
        ...
        // 19.2.9 EXDMA Offset Register (EDMOFR) - EXDMAC0にのみ存在
        ["EDMOFR"] = new RegisterDatasheetMetadata(0x00000000, 0xFFFFFFFF, "read-write"),
        ...
        // 19.2.12 EXDMA Status Register (EDMSTS) - 上記コメント参照
