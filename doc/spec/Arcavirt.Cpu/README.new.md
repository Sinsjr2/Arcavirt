...
- 観測性の実装方式(ログの出力先・フォーマット等)。既定では何も出力せず、
  デバッグ用の `registerLog` で明示的に指定した場合のみログを出す。
...
JSONファイルのトップレベル型。フィールドの型は「全体構造」のクラス図を、詳細は
以下の各節を参照。
...
2. 呼び出し側による上書きを、指定された順でさらに重ねる(同じマージ規則)。
...
`registerOverrides`は常にステップ5、つまりすべてのファイル・呼び出し側による上書きが
マージされた**後**に効く。個々のファイルが持つ`registerOverrides`同士は、ステップ1・2のマージ規則で
...
順序で上書きファイルを同じマージ規則でさらに重ねる。上書きファイル自体は
...
上書き適用後・ステップ6の検証完了後の状態)を、そのままJSONとして出力できる
インターフェースを持つこと。`includes`・`extends`・呼び出し側による上書き・
...
検出する実装上の配慮が必要になる(`includes`・呼び出し側による上書きをまたいだ
`to`キーの重複は意図的な上書きであり、検出対象ではない)。
...
どのパスにもマッチしない場合(`registerLog`の上書きファイル自体が読み込まれていない
...
呼び出し側による上書きで**一部の要素だけを追加・削除**したい場合は、配列ではなく
