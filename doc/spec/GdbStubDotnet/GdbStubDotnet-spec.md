...
- **アーキテクチャ方針**: コア**アーキテクチャ非依存**。同梱サンプルとして **RISC-V32** のスタブ実装を別アセンブリで提供（ライブラリ本体はアーキを内包しない）。
...
| # | 決定 | 根拠 |
|---|---|---|
| A-1 | ターゲット非依存の汎用フレームワーク。コアはアーキ非依存、RISC-V32 を別アセンブリのサンプルとして同梱。 | 用途を「C# エミュレータ」に限定せず再利用可能にする。 |
